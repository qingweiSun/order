...
import { ElemeShopEntity } from '@/entity/eleme-shop-entity';
...
export const Eleme = ({
  ...
  const [shopList, setShopList] = useState<ElemeShopEntity[]>([]);
  ...
  async function search() {
    ...
    try {
      const res = await fetch('/api/eleme/search', {
        method: 'POST',
        headers: {
  ...
  async function apply(shopEntity: ElemeShopEntity, day: number) {
    //判断是否选择了服务
    console.log(shopEntity);
    if (
      !shopEntity.shopAppItemOrder1.isSelected &&
      !shopEntity.shopAppItemOrder3.isSelected
    ) {
      toast.error('请选择服务');
      return;
    }
    setApplyIng(true);
    if (shopEntity.shopAppItemOrder1.isSelected) {
      try {
        const res1 = await fetch('/api/eleme/apply', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    if (shopEntity.shopAppItemOrder3.isSelected) {
      try {
        const res3 = await fetch('/api/eleme/apply', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  async function getOrderList() {
    try {
      const res = await fetch('/api/eleme/order', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async function pass(order: OrderEntity) {
    try {
      ...
      const res6 = await fetch('/api/eleme/pass', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async function rejection(order: OrderEntity) {
    try {
      ...
      setOrderList([...orderList]);
      const res6 = await fetch('/api/eleme/delete', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          <Card key={index}>
            <CardBody className={'flex flex-col gap-4'}>
              <div className={'flex flex-row justify-center items-center'}>
                ...
                <RenewalButton
                  isDisabled={
                    !shop.shopAppItemOrder1.isSelected &&
                    !shop.shopAppItemOrder3.isSelected
                  }
                  applyIng={applyIng}
                  apply={async (day) => {
              ...
              >
                <Checkbox
                  isSelected={shop.shopAppItemOrder1.isSelected}
                  onValueChange={(isSelected) => {
                    shop.shopAppItemOrder1.isSelected = isSelected;
                ...
                <div className={'text-gray-500 text-sm'}>
                  到期时间：
                  {formatTime(shop.shopAppItemOrder1.serviceValidDate)}
                </div>
              </div>
              <div
                className={'flex justify-between flex-col md:flex-row gap-1'}
              >
                <Checkbox
                  isSelected={shop.shopAppItemOrder3.isSelected}
                  onValueChange={(isSelected) => {
                    shop.shopAppItemOrder3.isSelected = isSelected;
                    setShopList([...shopList]);
                  }}
                >
                  评论回复
                ...
                <div className={'text-gray-500 text-sm'}>
                  到期时间：
                  {formatTime(shop.shopAppItemOrder3.serviceValidDate)}
                </div>
              </div>
            </CardBody>
          </Card>
        ...
        return (
          <Card key={index}>
            <CardBody className={'flex flex-col gap-4'}>
              ...
              <Divider />
              <div>
                {order.itemId == 1 && <div>续期类型：上报备餐</div>}
                {order.itemId == 3 && <div>续期类型：评论回复</div>}
